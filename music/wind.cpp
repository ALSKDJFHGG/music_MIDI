...
void Test()
{
    ...
    int test[] =
    {
        400,0,/*1*/M1,H1,M7,H1,M5,H1,M4,H1,M3,H1,M2,H1,M3,H1,M4,H1,/*2*/0,M1,H1,M7,H1,H5,H1,H4,H1,H3,H1,H2,H1,H3,H1,H4,/*3*/M1,H1,M7,H1,H5,H1,H4,H1,H3,H1,H2,H1,H3,H1,H4,H1,/*4*/M1,H1,M7,H1,H5,H1,H4,H1,
        _,_,_,M3,100,M3,100,M3,100,M3,
        MIDDLE_SPEED,X6,L6,M3,M1,M1,L4,M4,M1,X1,L5,M3,M1,L7,M1,M2,L5,X6,L6,M3,M1,M1,
        0,M1,H1,M7,H1,M5,H1,M4,H1,M3,H1,M2,H1,M3,H1,M4,H1,/*1*/HIGH_SPEED,M1,H1,M7,H1,H5,H1,H4,H1,H3,H1,H2,H1,H3,H1,H4,/*2*/
        X1,X2,X3,X4,X5,X6,X7,_,L1,L2,L3,L4,L5,L6,L7,_,M1,M2,M3,M4,M5,M6,M7,_,H1,H2,H3,H4,H5,H6,H7
    };
    ...
    for (auto i : test)
    {
        ...
        if (i == 100) { sleep = 190; continue; }
        if (i == _){ Sleep(1000); continue; }
...
void Hreat()
{
    HMIDIOUT handle;
    midiOutOpen(&handle, 0, 0, 0, CALLBACK_NULL);
    midiOutShortMsg(handle, 1 << 8 | 0xC0);
    int volume = 0x7f;
    int voice;
    int sleep = 350;
    ...
    int hreat[] =
    {
        /**/400,0,M1,H1,M7,H1,M5,H1,M4,H1,M3,H1,M2,H1,M3,H1,M4,H1,/**/M1,H1,M7,H1,H5,H1,H4,H1,H3,H1,H2,H1,H3,H1,H4,/**/0,M1,H1,M7,H1,H5,H1,H4,H1,H3,H1,H2,H1,H3,H1,H4,H1,/**/M1,H1,M7,H1,H5,H1,H4,H1
        ,300,M3,0,M3,0,M3,0,M3,M5,M5,700,M4,M3,300,M5,M3
    };
    for (auto i : hreat)
    {
        if (i == 0) { sleep = 180; continue; }
